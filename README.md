...
- 审核 Agent 只能通过结构化 `submit_review({ verdict, summary, findings?, checks? })` 给出结论。`changes-requested` 必须至少给出一条具体意见；`approved` 必须在 `checks` 里至少列出一条**自己动手做过的**核验（读了哪个文件、跑了什么命令、看到什么输出），否则一律被拒——说不出自己查过什么的通过，只是把作者的话复述了一遍。怀疑永远不比通过更费力：`changes-requested` 不要求 `checks`。
...
审核过程在 Thread 里完全可见：送审和审核意见都是普通消息，交付声明以 `deliverable.declared` 记录，审核结论以 `review.requested` / `review.submitted` / `review.rework` / `review.resolved` 事件记录（前两者带 `reviewType`，`review.submitted` 带审核者自查的 `checks`，界面上直接列出"审核者自己验证过的"）。
